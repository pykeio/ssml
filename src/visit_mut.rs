...
pub trait VisitMut<'s> {
	...
	fn visit_custom_mut(&mut self, node: &'s mut dyn CustomElement) {
...
pub fn visit_custom_mut<'s, V: VisitMut<'s> + ?Sized>(_v: &mut V, _node: &'s mut dyn CustomElement) {}
...
pub fn visit_speakable_mut<'s, V: VisitMut<'s> + ?Sized>(v: &mut V, node: &'s mut Element) {
	match node {
		...
		Element::Custom(node) => visit_custom_mut(v, node.as_mut())
	}
}
